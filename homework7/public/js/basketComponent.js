...
const basket = {
    ...
    methods: {
        ...
        addInObjectBasket(product) {
            ...
            if (find){//Если такой товар есть
                find.count++,//увеличиваем его количество на 1
                find.finishprice = find.count * find.price //пересчитываем сумму за товар сумму этим id
                fetch('/cart', {
                    method: 'PUT',
                    headers: {
                ...
                const good = Object.assign({count: 1, finishprice: product.price}, product)//создаем объект с количеством 1 и ценой равной продуктовой цены
        ...
        //Удаление товара из объекта корзины (когда количество 1)
        deleteFromObject(product){
            let find = this.cartItems.find((element) => element.id_product === product.id_product);
            //удаляем из корзины товаров 1 товар с порядкового номера това с выбранным id
            this.cartItems.splice(this.cartItems.indexOf(find),1)//удаляем из массива корзины 1 товар начиная с индекса выбранного товара
            console.log(this.cartItems)
            fetch('/cart', {
                method: 'DELETE',
                headers: {
    ...
    mounted(){ //начинаем с этого метода

        this.$parent.getJson(`/cart`)//запускаем метод из главного скрипта
            .then(data => {//берем оттуда данные в формате JSON С нашим массивом товаров
                let number = 0;
                for (let element of data){//Проходимся по ним в цикле
                    this.$data.cartItems.push(element);//добавляем каждый элемент в наш локальный массив продуктов
                    number += element.count;
                }
                console.log(number);
                this.$parent.counter=number;
            })
